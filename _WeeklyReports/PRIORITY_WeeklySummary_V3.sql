...
/* === Date anchors === */
DECLARE @StartOfYear     DATETIME = DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()) - 12, 0); -- 12 months ago (month start)
DECLARE @EndOfWeek       DATETIME = DATEADD(DAY, DATEDIFF(DAY, 0, GETDATE()) + 7, 0);      -- end of current week
DECLARE @StartNextMonth  DATETIME = DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()) + 1, 0);  -- first day of next month
DECLARE @TodayMidnight   DATETIME = DATEADD(DAY, DATEDIFF(DAY, 0, GETDATE()), 0);          -- today @ 00:00
...
VALUES ('COMP'), ('FLAGGED'), ('MISSED'), ('PENRVW'), ('PENDQA'), ('REVWD');
...
SELECT
    ...
    SUM(CASE 
        WHEN wo.targcompdate BETWEEN @TodayMidnight AND @EndOfWeek
        THEN 1 ELSE 0 
    END) AS [EOW],

    /* Work orders due by END of current month (i.e., before next month starts) */
    SUM(CASE 
    ...
    SUM(CASE 
        WHEN wo.targcompdate <> wo.fnlconstraint
         ...
         AND @TodayMidnight >= DATEADD(DAY, DATEDIFF(DAY, 0, wo.fnlconstraint) - 10, 0)
        THEN 1 ELSE 0
    END) AS [Risk Late],
    ...
    SUM(CASE 
        WHEN wo.targcompdate <= @StartNextMonth
         AND (
            (CURRENT_TIMESTAMP >= DATEADD(DAY, -10, wo.targcompdate) 
                AND wo.pluscfrequency IN ('1', '7', '14') AND wo.pluscfrequnit IN ('DAYS', 'MONTHS'))
            OR (wo.fnlconstraint < CURRENT_TIMESTAMP 
                AND CURRENT_TIMESTAMP > DATEADD(DAY, 8, wo.targcompdate) 
                AND wo.pluscfrequency BETWEEN 2 AND 5 AND wo.pluscfrequnit IN ('MONTHS'))
            OR (wo.fnlconstraint <= CURRENT_TIMESTAMP 
                AND CURRENT_TIMESTAMP > DATEADD(DAY, 36, wo.targcompdate) 
                AND wo.pluscfrequency BETWEEN 5 AND 11 AND wo.pluscfrequnit IN ('MONTHS'))
            OR (wo.fnlconstraint < CURRENT_TIMESTAMP 
                AND CURRENT_TIMESTAMP > DATEADD(DAY, 50, wo.targcompdate) 
...
WHERE wo.woclass IN ('WORKORDER','ACTIVITY')
  ...
  AND NOT EXISTS (
        SELECT 1 FROM @ExcludedStatus AS es
        WHERE es.status = wo.status
  )
