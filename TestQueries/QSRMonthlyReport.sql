...
Location / File Path: sql/work_orders/WO_Completion_Status_Tracking.sql
...
DECLARE @start_date AS DATETIME2 = '2025-08-01 00:00:00'  -- Start of target date range
DECLARE @end_date AS DATETIME2 = '2025-09-01 23:59:59'    -- END of target date range
...
FROM dbo.workorder AS w
    ...
    LEFT JOIN (
      ...
      ON w.siteid = comp_status.siteid AND w.wonum = comp_status.wonum 
        AND comp_status.rn = 1  -- Most recent completion status only
...
WHERE 
    ...
    AND (w.reportdate IS NULL OR w.reportdate >= '2024-01-01')
